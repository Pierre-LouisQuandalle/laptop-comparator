<script lang="ts">
    ...
    let { laptop, isRecommended = false }: Props = $props();
  
    let perfomanceColor = $derived.by(function (){
        if (laptop.performance >= 80) return "bg-green-500";
        if (laptop.performance >= 60) return "bg-yellow-500";
        return "bg-red-500";
  ...
  <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 hover:shadow-xl transition-all duration-300 max-w-sm
    ...
    <div class="space-y-2 mb-4">
      ...
      <div class="flex items-center gap-2">
        <Icon icon="mdi:memory" class="text-gray-800 dark:text-gray-200" />
        <span class="text-gray-700 dark:text-gray-300">{laptop.ram}</span>
      </div>
      
      <div class="flex items-center gap-2">
        <Icon icon="mdi:harddisk" class="text-gray-800 dark:text-gray-200" />
        <span class="text-gray-700 dark:text-gray-300">{laptop.storage}</span>
      </div>
    </div>
    ...
    <div class="mb-2">
      <div class="flex justify-between items-center mb-1">
        ...
        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{laptop.performance}%</span>
      </div>
      <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
        <div
          class="h-2.5 rounded-full transition-all duration-300 {perfomanceColor}"
